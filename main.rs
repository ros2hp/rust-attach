    ...
    let mut tasks_spawned : usize = 0;
        ...
        while more_items {
                                ...
                                if scan_output.items != None {
                                    ...
                                    // go persist_task(..)
                                    tokio::spawn(persist_task(tasks_spawned, client, scan_output.items.unwrap(), table_name, task_ch.clone()));
                                    persist_tasks += 1;
                                    tasks_spawned += 1;
                                }
        ...
        println!("Max Tasks: {} Elapsed Time: {} secs", MAX_TASKS, elapsed.as_secs());
    ...
    match bat_w_outp {
        ...
        Ok(resp) => {
            ...
            if resp.unprocessed_items.as_ref().unwrap().len() > 0 {
                println!("**** persist_batch: unprocessed items {}",resp.unprocessed_items.unwrap().values().len());
            }
        }
    }
